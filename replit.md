...
- **Attendee Portal Preview (Spoof Mode)**: Organization owners can preview the attendee portal exactly as a specific attendee would see it. Access via "Preview Portal As Attendee" button in the attendee details sheet. Opens portal in new tab with amber preview banner, disabled editing/logout, and visibility conditions evaluated for that attendee. Only organization owners can access this feature. Uses `/api/organizations/:orgId/attendees/:attendeeId/spoof-portal` endpoint with owner-only authorization.
