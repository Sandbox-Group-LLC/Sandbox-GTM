...
- **Thought Leadership / Blog**: Public-facing article pages at `/thought-leadership` (listing) and `/thought-leadership/:slug` (individual article). Manual article management via admin UI at `/admin/thought-leadership` (super-admin only). Public API endpoints: `GET /api/public/thought-leadership/articles` (list published), `GET /api/public/thought-leadership/articles/:slug` (single article). Admin API endpoints (authenticated, super-admin): `GET/POST /api/thought-leadership/articles`, `GET/PATCH/DELETE /api/thought-leadership/articles/:id`. Articles upsert by slug, auto-calculate read time, and support HTML/Markdown content. Schema: `thoughtLeadershipArticles` table.
