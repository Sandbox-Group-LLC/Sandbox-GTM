...
- **Thought Leadership / Blog**: Public-facing article pages at `/thought-leadership` (listing) and `/thought-leadership/:slug` (individual article). Supports automated content publishing via BabyLoveGrowth webhook integration and manual article management via admin UI at `/admin/thought-leadership` (super-admin only). Public API endpoints: `GET /api/public/thought-leadership/ping` (health check), `GET /api/public/thought-leadership/articles` (list published), `GET /api/public/thought-leadership/articles/:slug` (single article), `POST /api/public/thought-leadership/publish` (webhook for BabyLoveGrowth, Bearer token auth via `BABYLOVEGROWTH_API_KEY`). Admin API endpoints (authenticated, super-admin): `GET/POST /api/thought-leadership/articles`, `GET/PATCH/DELETE /api/thought-leadership/articles/:id`. Articles upsert by slug, auto-calculate read time, and support HTML/Markdown content. Schema: `thoughtLeadershipArticles` table.
