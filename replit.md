...
- **Byword.ai Integration**: AI article generation via Byword API (`server/byword.ts`). Two-step process: create article → poll for completion (~60s). Supports keyword mode (Byword generates title + article) and title mode (user provides title). Admin UI has "Generate with Byword" button with loading state. Requires `BYWORD_API_KEY` secret and `BYWORD_DOMAIN` env var (set to sandbox-gtm.com).
