...
export class DatabaseStorage implements IStorage {
  ...
  async deleteEvent(organizationId: string, id: string): Promise<void> {
    ...
    // Delete session check-ins BEFORE attendees (sessionCheckIns.attendeeId references attendees.id)
    await db.delete(sessionCheckIns).where(eq(sessionCheckIns.eventId, id));
    ...
    // roomOpenHours references sessionRooms.id via roomId, so delete via subquery
    await db.delete(roomOpenHours).where(
      sql`${roomOpenHours.roomId} IN (SELECT id FROM session_rooms WHERE event_id = ${id})`
    );
