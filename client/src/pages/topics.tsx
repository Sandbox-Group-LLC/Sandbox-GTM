...
export default function Topics() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: TopicFormData) => {
      return await apiRequest("POST", "/api/session-topics", data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/session-topics"] });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: TopicFormData }) => {
      return await apiRequest("PATCH", `/api/session-topics/${id}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/session-topics"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return await apiRequest("DELETE", `/api/session-topics/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/session-topics"] });
  ...
  const handleEdit = (topic: SessionTopic) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader
        ...
        breadcrumbs={[{ label: "Content Experiences", href: "/sessions" }, { label: "Topics" }]}
        actions={
          <Button size="sm" onClick={openAddDialog} data-testid="button-add-topic">
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {topics.length === 0 ? (
            <EmptyState
              icon={Lightbulb}
              title="No topics yet"
              description="Create topics to categorize sessions and enable personalized recommendations for attendees"
              action={{
                label: "Create Topic",
      ...
      <Dialog open={isDialogOpen} onOpenChange={(open) => open ? setIsDialogOpen(true) : handleDialogClose()}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingTopic ? "Update the topic details below" : "Create a new topic to categorize sessions and enable personalized recommendations"}
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g., Strategy, Technology, Leadership" {...field} data-testid="input-topic-name" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder="Brief description of this topic" {...field} data-testid="input-topic-description" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
