...
export default function Vendors() {
  ...
  // Filter vendors by selected event
  const filteredVendors = selectedEventId
    ? vendors.filter((v) => v.eventId === selectedEventId)
    : [];

  // Filter budget items by selected event for total spend calculation
  const filteredBudgetItems = selectedEventId
    ? budgetItems.filter((item) => item.eventId === selectedEventId)
    : [];

  const totalForecastCost = filteredBudgetItems.reduce((sum, item) => sum + parseFloat(item.forecastAmount || "0"), 0);
  const activeContracts = filteredVendors.filter((v) => v.contractStatus === "active").length;
  const pendingApproval = filteredVendors.filter((v) => v.approvalStatus === "pending").length;
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader title="Vendors" breadcrumbs={[{ label: "Vendors" }]} />
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {!selectedEventId ? (
            <EmptyState
              ...
              description="Choose an event to view and manage its vendors"
            />
          ) : (
            <>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold" data-testid="text-active-vendors">{filteredVendors.length}</div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between gap-2">
            ...
            <Dialog open={isAddDialogOpen} onOpenChange={(open) => {
              if (open) {
                setNewVendorEventId(selectedEventId);
              }
