...
import { Users, Zap, Target, CalendarDays, Monitor, ArrowRight, RefreshCw, CheckCircle, AlertCircle } from "lucide-react";
      ...
      <main className="flex-1 p-6 max-w-6xl mx-auto w-full space-y-6">
        {connections.length === 0 ? (
          <Card className="border-amber-200 bg-amber-50 dark:bg-amber-950/20">
            <CardContent className="flex items-center gap-3 py-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">Connect a registration platform to start syncing attendees.</p>
              </div>
              <Link href="/connect"><Button size="sm" variant="outline">Connect Now</Button></Link>
            </CardContent>
          </Card>
        ) : (
          <Card className="border-green-200 bg-green-50 dark:bg-green-950/20">
            <CardContent className="flex items-center gap-3 py-4">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium">Connected to {connections[0]?.name}</p>
                <p className="text-xs text-muted-foreground capitalize">{connections[0]?.adapter} · {events.length} event{events.length !== 1 ? "s" : ""} synced</p>
              </div>
              {activeEvent && (
                <Button size="sm" variant="outline" onClick={() => handleSync(activeEvent.id)}>
                  <RefreshCw className="h-3 w-3 mr-1" />Sync Attendees
                </Button>
              )}
            </CardContent>
          </Card>
        ...
        {events.length > 0 && (
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline">{activeEvent?.name}</Badge>
          </div>
        )}
        ...
        {liveMoments.length > 0 && (
          <Card className="border-blue-200 bg-blue-50 dark:bg-blue-950/20">
            <CardContent className="flex items-center gap-3 py-4">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium">{liveMoments.length} moment{liveMoments.length !== 1 ? "s" : ""} live right now</p>
                <p className="text-xs text-muted-foreground">{liveMoments.map((m: any) => m.title).join(", ")}</p>
              </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Checked In" value={checkinStats?.checkedIn ?? 0} sub={`of ${checkinStats?.totalAttendees ?? 0} registered`} icon={<Users className="h-4 w-4 text-muted-foreground" />} href="/check-in" loading={!checkinStats && !!activeEvent} />
          <StatCard title="Interactions Today" value={interactionStats?.interactionsToday ?? 0} sub={`${interactionStats?.totalInteractions ?? 0} total`} icon={<Target className="h-4 w-4 text-muted-foreground" />} href="/check-in" loading={!interactionStats && !!activeEvent} />
          <StatCard title="Active Moments" value={liveMoments.length} sub={`${moments.length} total`} icon={<Zap className="h-4 w-4 text-muted-foreground" />} href="/moments" loading={false} />
...
function StatCard({ title, value, sub, icon, href, loading }: { title: string; value: number; sub: string; icon: React.ReactNode; href: string; loading: boolean }) {
  return (
    <Link href={href}>
      <Card className="cursor-pointer hover:border-primary/50 transition-colors">
        ...
        <CardContent>
          {loading ? <Skeleton className="h-8 w-16" /> : <p className="text-2xl font-bold">{typeof value === "number" ? value.toLocaleString() : value}</p>}
