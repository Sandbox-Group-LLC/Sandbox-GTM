...
router.get("/", async (req: Request<EP>, res: Response) => {
  try {
    ...
    const conditions: any[] = [eq(meetings.eventId, eventId)];
    ...
    const rows = await db.select({ meeting: meetings, attendee: attendees })
      .from(meetings)
...
router.get("/stats", async (req: Request<EP>, res: Response) => {
  try {
    ...
    res.json({ totalMeetings: total.count, pendingResponses: pending.count, outcomesCaptured: withOutcome.count, highIntent: highIntent.count });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});

router.patch("/:meetingId", async (req: Request<EP & { meetingId: string }>, res: Response) => {
  try {
...
router.post("/:meetingId/outcome", async (req: Request<EP & { meetingId: string }>, res: Response) => {
  try {
